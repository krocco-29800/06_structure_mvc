...
if ($db){
   ...
   $sql = $db->prepare("SELECT comment.*,contact.firstname,contact.lastname, contact.created_at FROM comment,contact WHERE comment.post_id='".$post_id."' AND comment.user_id=contact.user_id ORDER BY id DESC");
   $sql->execute();
   $comments = $sql->fetchAll(PDO::FETCH_ASSOC);
}
// Utils::dump_die($comments);
// ma logique de controller
// blah blah blah
// On charge la vue
